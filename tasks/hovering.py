...
class Hovering(Task):
    ...
    def get_reward(self):
        ...
        z_distance = abs(self.sim.pose[2] - self.target_pos[2]) #[1,0] ; 1 if distance is maximal; 0 if target is arrived at

        # base reward
        reward = 0
        penalties = 0
        penalties += distance**2
        ...
        if distance < 5:
            reward += 100
        if self.sim.time >= self.runtime and distance < 5:
            reward +=100
